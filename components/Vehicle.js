import { useBox, useRaycastVehicle, useCompoundBody } from '@react-three/cannon'
import { useFrame } from '@react-three/fiber'
import { useEffect, useRef, forwardRef } from 'react'
import { useControls } from '@/lib/use-controls'
import { Box, Cylinder } from '@react-three/drei'

export const Wheel = forwardRef(function Wheel({ leftSide, radius = 0.7, ...props }, ref) {
  useCompoundBody(
    () => ({
  ...
  return (
    <group ref={ref}>
      <Cylinder castShadow receiveShadow args={[radius, radius, 0.5, 16]} rotation={[0, 0, -Math.PI / 2]}>
...
function Vehicle({
  ...
  return (
    <group ref={vehicle}>
      <Box ref={chassisBody} args={[1.7, 1, 4]} />
